/*
  FrSky single wire serial class for Teensy 3.x and 328P based boards (e.g. Pro
  Mini, Nano, Uno)
  (c) Pawelsky 20150725
...
#if defined(__MK20DX128__) || defined(__MK20DX256__)
  if (id == SERIAL_USB) // Not really single wire, but added for debug purposes
                        // via USB
  {
    port = &Serial;
    Serial.begin(57600);
...
#else
#error                                                                         \
    "Unsupported processor! Only Teesny 3.x and 328P based processors supported.";
#endif
  crc = 0;
}
