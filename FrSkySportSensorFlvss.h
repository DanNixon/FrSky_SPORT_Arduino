...
public:
  ...
  void setData(float cell1, float cell2 = 0.0, float cell3 = 0.0,
               float cell4 = 0.0, float cell5 = 0.0, float cell6 = 0.0);
  virtual void send(FrSkySportSingleWireSerial &serial, uint8_t id,
                    uint32_t now);

private:
  static uint32_t setCellData(uint8_t cellNum, uint8_t firstCellNo, float cell1,
                              float cell2);
  uint32_t cellData1;
  uint32_t cellData2;
  uint32_t cellData3;
